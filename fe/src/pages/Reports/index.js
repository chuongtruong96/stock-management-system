...
export default function Reports({ language="en" }) {
  ...
  const fetchReport = async () => {
    try{
      ...
      if(!data.length) toast.info(language==="vi" ? 
  ...
  const exportFile = async (ext) => {
    try{
      const { data } = await api.get(`/reports/export/${ext}`, {
        params:{ month:`${year}-${month}` }, responseType:"blob"
      });
  ...
  return (
    <DashboardLayout>
      ...
      <MDBox pt={6} pb={3}>
        ...
        {/* --- controls --- */}
        <MDBox display="flex" gap={2} flexWrap="wrap" mb={3} mt={2}
                 justifyContent="space-between" alignItems="center" mx={2}>
          <FormControl size="small" sx={{minWidth:120}}>
            <InputLabel>{language==="vi" ? "Tháng" : "Month"}</InputLabel>
            <Select value={month} label="Month"
