...
const UnitManagement = ({ language }) => {
    ...
    const handleEditUnit = async () => {
        try {
            ...
            setUnits(units.map((unit) => (unit.id === editUnit.id ? response.data : unit)));
            setEditUnit(null);
            setOpenDialog(false);
        } catch (error) {
            alert(language === 'vi' ? 'Lỗi khi cập nhật đơn vị' : 'Error updating unit');
        }
    };
    ...
    const handleDeleteUnit = async (unitId) => {
        try {
            ...
            alert(language === 'vi' ? 'Lỗi khi xóa đơn vị' : 'Error deleting unit');
        }
    };

    const columns = [
        { field: 'nameEn', headerName: language === 'vi' ? 'Tên (Tiếng Anh)' : 'Name (English)', width: 200 },
        { field: 'nameVn', headerName: language === 'vi' ? 'Tên (Tiếng Việt)' : 'Name (Vietnamese)', width: 200 },
        {
            field: 'action',
            ...
            renderCell: (params) => (
                <>
                    <Button
                        variant="contained"
                        color="primary"
                        onClick={() => { setEditUnit(params.row); setOpenDialog(true); }}
                        sx={{ mr: 1 }}
                    >
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Dialog open={openDialog} onClose={() => { setOpenDialog(false); setEditUnit(null); }}>
                <DialogTitle>{editUnit ? (language === 'vi' ? 'Sửa Đơn Vị' : 'Edit Unit') : (language === 'vi' ? 'Thêm Đơn Vị' : 'Add Unit')}</DialogTitle>
                <DialogContent>
                    <TextField
                        label={language === 'vi' ? 'Tên (Tiếng Anh)' : 'Name (English)'}
                        fullWidth
                        margin="normal"
                    ...
                    />
                    <TextField
                        label={language === 'vi' ? 'Tên (Tiếng Việt)' : 'Name (Vietnamese)'}
                        fullWidth
                        margin="normal"
                ...
                <DialogActions>
                    <Button onClick={() => { setOpenDialog(false); setEditUnit(null); }}>
                        {language === 'vi' ? 'Hủy' : 'Cancel'}
                    </Button>
                    <Button onClick={editUnit ? handleEditUnit : handleAddUnit} color="primary">
                        {editUnit ? (language === 'vi' ? 'Cập Nhật' : 'Update') : (language === 'vi' ? 'Thêm' : 'Add')}
                    </Button>
                </DialogActions>
            </Dialog>
        </Box>
