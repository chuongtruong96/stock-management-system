...
export default function EnhancedUserManagement() {
  ...
  // Compact header component with purple/pink color scheme
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" sx={{ opacity: 0.9, fontSize: '0.875rem' }}>
              {t("manageUsersDescription") || "Manage system users and permissions"}
            </Typography>
          </Box>
        </Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Typography variant="h6" fontWeight="600" sx={{ color: 'text.primary', fontSize: '1.1rem' }}>
            {t("userInventory") || "User Inventory"}
          </Typography>
          <Chip 
            label={`${rows.length} ${t("users") || "users"}`}
            size="small"
            sx={{ 
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setRows((u) => u.filter((x) => x.id !== deleteDialog.user.id));
  ...
  // Get department name
  // Get department name with better handling for null/undefined values
  const getDepartmentName = useCallback((departmentId) => {
    if (!departmentId) {
      return i18n.language === 'vi' ? "Chưa phân công" : "Unassigned";
    }
    const dept = departments.find(d => d.id === departmentId);
    return dept?.name || (i18n.language === 'vi' ? "Phòng ban không tồn tại" : "Department not found");
  ...
  const columns = [
    {
      ...
      renderHeader: () => (
        <CustomColumnHeader 
          icon={Person} 
          title={i18n.language === 'vi' ? 'Avatar' : 'Avatar'}
        />
      ),
      renderCell: ({ row }) => (
    ...
    },
    {
      ...
      renderHeader: () => (
        <CustomColumnHeader 
          icon={Person} 
          title={i18n.language === 'vi' ? 'Người dùng' : 'User'}
        />
      ),
      renderCell: ({ row }) => (
        <Box>
    ...
    },
    {
      ...
      renderHeader: () => (
        <CustomColumnHeader 
          icon={RoleIcon} 
          title={i18n.language === 'vi' ? 'Vai trò' : 'Role'}
        />
      ),
      renderCell: ({ row }) => (
        <Chip
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Typography 
          ...
          title={getDepartmentName(row.departmentId)}
        >
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', gap: 1.5, justifyContent: 'flex-end' }}>
          <Tooltip title={t("editUser") || "Edit User"} arrow>
            <IconButton
              size="small"
              onClick={() => {
                setDraft({
          ...
          <Tooltip title={t("deleteUser") || "Delete User"} arrow>
            <IconButton
              size="small"
              onClick={() => remove(row)}
  ...
  return (
    <AdminLayout titleKey="userManagement" icon="person">
      ...
      <ConfirmDialog
        ...
        message={
          deleteDialog.user 
            ? `${t("confirmDeleteUserMessage") || "Are you sure you want to delete user"} "${deleteDialog.user.username}"? ${t("actionCannotBeUndone") || "This action cannot be undone."}`
            : ""
        }
