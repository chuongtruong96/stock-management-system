...
const OrderHistory = ({ language }) => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                const departmentId = userInfo.data.employee.department.departmentId;
    ...
    const orderColumns = [
        ...
        { field: 'createdAt', headerName: language === 'vi' ? 'Ngày Tạo' : 'Date', width: 200 },
        { field: 'status', headerName: language === 'vi' ? 'Trạng Thái' : 'Status', width: 150 },
        { field: 'adminComment', headerName: language === 'vi' ? 'Bình Luận Quản Trị' : 'Admin Comment', width: 300 },
        {
            field: 'action',
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Dialog open={!!selectedOrder} onClose={() => setSelectedOrder(null)}>
                <DialogTitle>
                    {language === 'vi' ? `Chi Tiết Đơn Hàng (Mã: ${selectedOrder?.orderId})` : `Order Details (ID: ${selectedOrder?.orderId})`}
                </DialogTitle>
                <DialogContent>
                    <Typography>{language === 'vi' ? 'Trạng Thái' : 'Status'}: {selectedOrder?.status}</Typography>
                    <Typography>{language === 'vi' ? 'Ngày Tạo' : 'Date'}: {selectedOrder?.createdAt}</Typography>
                    {selectedOrder?.adminComment && (
                        <Typography>{language === 'vi' ? 'Bình Luận Quản Trị' : 'Admin Comment'}: {selectedOrder.adminComment}</Typography>
                    )}
